...
This is Copilot's `AGENTS.md`, the file from the React lessons. Prefer that name? VS Code setting `chat.useAgentsMdFile`.
...
- Generic answer about Expo? Check the path: `.github/copilot-instructions.md`.

Thirty seconds. Do it every time you start a new project.
...
Each one becomes **a line in `copilot-instructions.md`**. Commit it.
...
C may be homework. D and E are always in class.
