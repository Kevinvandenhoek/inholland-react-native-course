...
Story 1 is a worked example. Replace it with your own.>

### 1. Search Pokémon by name

As a user I can search Pokémon by name so that I find one without scrolling.

- [ ] The list screen has a search field at the top, placeholder "Search".
...
- [ ] Data comes from the list already loaded with TanStack Query (`/pokemon?limit=151`); no extra request per keystroke.
- [ ] Loading state and error state (same as the list).
